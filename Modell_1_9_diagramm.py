...
if simulation_starten:
    ...
    with ergebnis_spalte:
        ...
        p80_pfad = np.percentile(werte, 80, axis=1)
        median_pfad = np.median(werte, axis=1)

        fig_band = go.Figure()

        # Obere Grenze des Unsicherheitsbereichs
        fig_band.add_trace(
            go.Scatter(
                x=zeitachse,
                y=p80_pfad,
    ...
    st.info(
        ...
        "Noch nicht enthalten sind Inflation, Gebühren, Rebalancing, "
    ...
    with info_spalte:
        st.markdown("### Überblick")
        st.caption("Hier erscheinen später Annahmen, Warnungen und Zusatzinfos.")